...
On the other hand, the objective of motion generation task is to synthesize a mechanism, which can guide a rigid body along a prescribed motion\footnote{motion is the combination of position and orientation.}.
...
The prior step is called Type Synthesis, and the later step is Dimension Synthesis.
...
These points, instead of the actual task, are now treated as interpolation or approximation task for mechanism synthesis.
...
For this type of problem, only finite solutions exist and in this case, only one solution is obtained as shown in Fig.~\ref{circuit_defect}.
It can be seen that, although coupler of the four-bar passes exactly through five precision positions, it can not do it without changing the circuit.
...
Even if the defect-free solution is found, the coupler motion in-between the precision points may not have the desired properties.
...
%As a guiding direction to advance state of the art, we propose following criteria for an effective mechanism synthesis method,
...
In section~\ref{subsec_motiongen}, we present the obtained solutions for the prescribed motion presented in Fig.~\ref{circuit_defect}.
...
However, these methods tend to be slow and inefficient, as they start from scratch for every instance, and with no guarantee of converging to an acceptable solution.
...
Wu et al.\cite{wu2011} presented a method based on finite Fourier series for open and closed path generation, but it is specific to four-bar mechanisms.
...
%  ---
\subsection{The Method Overview}
...
2) we exploit the nonlinear nature of the relationship between the linkage parameters and coupler motions to create a sensitive, wide-ranging, compact, and efficient database with hierarchical clustering.
...
In this paper, we employ pattern recognition and computational shape analysis, to create a similarity transform-independent signature for coupler path and motion.
When a user provides the prescribed motion or path, a query representing invariant signature \footnote{path or motion signature} of the input is raised for $k$ nearest neighbors among cluster centers in the database.
...
The prescribed motion entails this information.
However, precision point approach cannot capture it.
...
We solve the formulation by employing machine-learning techniques and generate a large number of defect-free conceptual designs.

...
Lets consider two coupler paths as shown in~\ref{wholePart}.
Let $p$ be path signature of such a part path which acts as template while $W$ is the path signature corresponding to a path (Whole) that can completely embed the template as shown in Fig.~\ref{pathSignature}. The orientation information shown in Fig.~\ref{wholePart} is ignored for path matching. It will be used later for matching of motion signature.
